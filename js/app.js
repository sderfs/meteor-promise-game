  ...
  const PageRenderer = {
    ...
    renderBilibiliHome(page) {
      ...
      (page.data.videos || []).forEach(video => {
        ...
        thumb.style.cssText = 'width:120px;height:68px;background:#E6E2D3;border-radius:6px;flex-shrink:0;overflow:hidden;';
        if (video.thumb) {
        ...
        data.journals.forEach(function(journal) {
          ...
          if (journal.locked && !window.gameData.state['qq_journal_unlocked_' + page.id]) {
            ...
            jHeader.addEventListener('click', function(e) {
              e.stopPropagation();
              // 弹出密保验证弹窗（自建独立弹层，避免广告屏蔽插件按 #modal / role / aria 规则隐藏）
              var overlay = document.createElement('div');
              overlay.style.cssText = 'position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.4);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);z-index:1000;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease;';
              document.body.appendChild(overlay);
              var closeModal = function() { if (overlay && overlay.parentNode) overlay.parentNode.removeChild(overlay); };
              overlay.addEventListener('click', function(ev) { if (ev.target === overlay) closeModal(); });
              overlay.addEventListener('touchend', function(ev) { if (ev.target === overlay) { ev.preventDefault(); closeModal(); } }, { passive: false });
              var modal = overlay;
